...
object Streaming{
	...
	def main(args: Array[String]){
		...
		var outputPRDD: org.apache.spark.rdd.RDD[(Int, (String, String))] = null
		...
		streamAll.foreachRDD({ rdd =>
			if(!rdd.isEmpty()){
				...
				inputPRDD.cache
				inputPRDD.count
				missedKeys = inputPRDD.keys.distinct
				//missedKeys.count

				println("missedKeys count: " + missedKeys.count)
				...
				DB_PRDD = sc.parallelize(db_arr)

				outputPRDD = inputPRDD.join(DB_PRDD)
				println("output count: " + outputPRDD.count)
				//outputPRDD.take(1).foreach(println)
				
				var t1 = System.currentTimeMillis

				println("stream time: " + (t1 - t0) + " ms")
