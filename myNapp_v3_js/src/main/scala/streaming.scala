...
object Streaming{
	...
	def main(args: Array[String]){
		...
		val ssc = new StreamingContext(sc, Milliseconds(150))
...
*/
				...
				var inputPRDD = rdd.map{ s=>  var tmp = s.split('|'); (tmp.lift(1).get.toInt, s) }//.partitionBy(hashP)

				inputPRDD.cache
				//inputPRDD.count
				println("data|in|input: " + inputPRDD.count)
...
/* miss data join thread */
				val missedFuture = Future{ // for missed data
					...
					if(isCogroup){
						...
						missedKeysCount = missedKeys.count
					...
					if(!isEmpty_missedData){
						DB_PRDD = missedKeys.mapPartitions({ iter =>
							var count = 0
...
// cache management thread - update LRU keys
				var LRUKeyThread = new Thread(){
					override def run = {
						...
						var inputKeysRDD = inputPRDD.mapPartitions({ iter =>
					...
					override def run = {						
						...
						if(isCogroup){
							...
							if(isEmpty_missedData_th){
								...
								cacheTmp = cachedPRDD.subtractByKey(removeList, hashP).union(DB_PRDD).partitionBy(hashP)
								//flag = true
								isPerformed_CC_PrevIter = true
							}else{
