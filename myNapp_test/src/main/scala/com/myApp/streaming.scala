...
object Streaming{
	...
	def main(args: Array[String]){
		...
		var isCogroup = true
		var enableCacheCleaningFunction = false
...
/* cache management thread - update cachedRDD */
				cacheThread = new Thread(){
					override def run = {						
						...
						if(enableCacheCleaningFunction_th == false){ // disable cache cleaning
							if(streamingIteration == 1){
								cacheTmp = DB_PRDD.partitionBy(hashP)
							}else{
				...
				t0 = System.currentTimeMillis
				if(streamingIteration == 1){
					outputCount = missedPRDD.count
				}else if(isEmpty_missedData){
