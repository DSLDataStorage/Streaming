...
object Streaming{
	...
	def main(args: Array[String]){
		...
		var cachedPRDD: org.apache.spark.rdd.RDD[(Long, String)] = null
		var cacheTmp: org.apache.spark.rdd.RDD[(Long, String)] = null		// for cache update
				
		var DB_PRDD: org.apache.spark.rdd.RDD[(Long, String)] = null

		var LRU_RDD: org.apache.spark.rdd.RDD[(Long, Int)] = null
		var LRU_Tmp: org.apache.spark.rdd.RDD[(Long, Int)] = null			// for LRUKey update
		
		var cogroupedRDD: org.apache.spark.rdd.RDD[(Long, (Iterable[String], Iterable[String]))] = null
		var joinedPRDD_hit: org.apache.spark.rdd.RDD[(Long, (String, String))] = null
		var joinedPRDD_missed: org.apache.spark.rdd.RDD[(Long, (String, String))] = null
				
		var missedPRDD: org.apache.spark.rdd.RDD[(Long, String)] = null
		var missedPRDD_cog_form: org.apache.spark.rdd.RDD[(Long, (Iterable[String], Iterable[String]))] = null
		var missedKeys: org.apache.spark.rdd.RDD[Long] = null
		
		var removeList: org.apache.spark.rdd.RDD[(Long, Int)] = null
...
0
		...
		//var preCachedFile = sc.textFile("file:///home/user/spark/cached40k.tbl", 16)
		//cachedPRDD = preCachedFile.map({s=>var k = s.split('|'); var k2 = k(0).split(' '); (k2(0).toInt, k(1).toString)}).partitionBy(hashP)

		var preCachedFile = sc.textFile("file:///home/user/spark/cached40k_weather.tbl", 16)
		cachedPRDD = preCachedFile.map({s=>var k = s.split('|'); (k(0).toLong, s)}).partitionBy(hashP)
...
*/
				...
				var inputPRDD = rdd.map{ s=>  var tmp = s.split('|'); (tmp.lift(0).get.toLong, s) }
				//println("data|input|input count: " + inputPRDD.count)
...
/* miss data join thread */
				val missedFuture = Future{ // for missed data
					...
					if(!isEmpty_missedData){
						//println("missssssssssssssssssssssssssssssssssss")
						...
						DB_PRDD = missedPRDD_cog_form.mapPartitions({ iter =>						
							...
							val db = mongoClient("weather")
							//val db = mongoClient("n4s1000h")
							...
							if(!iter.isEmpty){
								iter.foreach{ case(k, (v1, v2)) =>
									...
									var tmp = MongoDBObject("weatherkey" -> k);
									if(count == 0){
										qList = List(tmp)
										count = 1
									}else{
							...
							println(dbData(0))
							var db_arr = dbData.map{ s=> {var tmpAll=s.split(" "); (tmpAll.lift(10).get.toLong, s)} }
							db_arr.iterator
...
// cache management thread - update LRU keys
				...
				RemoveListThread = new Thread(){
					override def run = {
						...
						else if(streamingIteration_th > 2){
...
// ------------------------------------------------ main thread ------------------------------------------------ 
				...
				//println(joinedPRDD_missed.first)
